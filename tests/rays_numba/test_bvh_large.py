...
def test_make_bvh_grid():
    ...
    5. It calls the `make_bvh_grid()` function with these parameters.
    ...
    # Call the function
    bvh_grid = make_bvh_grid(min_xyz, max_xyz, poly_pts, poly_tri, step)

    # Assertions
    assert len(bvh_grid) == 64, "The grid should have 64 cells (4x4x4)"

    # Check if corner cells contain the correct polygons
    assert set(bvh_grid[(0, 0, 0)]) == {0, 2, 4}, "Corner (0,0,0) should have polygons 0, 2, and 4"
    assert set(bvh_grid[(3, 0, 0)]) == {0, 2, 5}, "Corner (3,0,0) should have polygons 0, 2, and 5"
    assert set(bvh_grid[(0, 3, 0)]) == {0, 3, 4}, "Corner (0,3,0) should have polygons 0, 3, and 4"
    assert set(bvh_grid[(3, 3, 0)]) == {0, 3, 5}, "Corner (3,3,0) should have polygons 0, 3, and 5"
    assert set(bvh_grid[(0, 0, 3)]) == {1, 2, 4}, "Corner (0,0,3) should have polygons 1, 2, and 4"
    assert set(bvh_grid[(3, 0, 3)]) == {1, 2, 5}, "Corner (3,0,3) should have polygons 1, 2, and 5"
    assert set(bvh_grid[(0, 3, 3)]) == {1, 3, 4}, "Corner (0,3,3) should have polygons 1, 3, and 4"
    assert set(bvh_grid[(3, 3, 3)]) == {1, 3, 5}, "Corner (3,3,3) should have polygons 1, 3, and 5"

    # Check if center cells contain no polygons
    assert len(bvh_grid[(1, 1, 1)]) == 0, "Center cell (1,1,1) should contain no polygons"
    assert len(bvh_grid[(2, 2, 2)]) == 0, "Center cell (2,2,2) should contain no polygons"

    # Check if edge cells contain the correct number of polygons
    assert len(bvh_grid[(0, 1, 1)]) == 1, "Edge cell (0,1,1) should contain 1 polygon"
    assert len(bvh_grid[(3, 1, 1)]) == 1, "Edge cell (3,1,1) should contain 1 polygon"
    assert len(bvh_grid[(1, 0, 1)]) == 1, "Edge cell (1,0,1) should contain 1 polygon"
    assert len(bvh_grid[(1, 3, 1)]) == 1, "Edge cell (1,3,1) should contain 1 polygon"
    assert len(bvh_grid[(1, 1, 0)]) == 1, "Edge cell (1,1,0) should contain 1 polygon"
    assert len(bvh_grid[(1, 1, 3)]) == 1, "Edge cell (1,1,3) should contain 1 polygon"
