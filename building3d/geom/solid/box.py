...
def box(
    ...
    poly_w0 = Polygon(np.vstack((p0, p1, p5, p4)), name="wall-0")
    poly_w1 = Polygon(np.vstack((p1, p2, p6, p5)), name="wall-1")
    poly_w2 = Polygon(np.vstack((p3, p7, p6, p2)), name="wall-2")
    poly_w3 = Polygon(np.vstack((p0, p4, p7, p3)), name="wall-3")
    poly_rf = Polygon(np.vstack((p4, p5, p6, p7)), name="roof")

    wall_fl = Wall([poly_fl], name="floor")
    wall_w0 = Wall([poly_w0], name="wall-0")
    wall_w1 = Wall([poly_w1], name="wall-1")
    wall_w2 = Wall([poly_w2], name="wall-2")
    wall_w3 = Wall([poly_w3], name="wall-3")
    wall_rf = Wall([poly_rf], name="roof")
